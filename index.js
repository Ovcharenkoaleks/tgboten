...
bot.on('text', async (ctx) => {
    ...
    const quizOptions = {
      ...
      correct_option_id: 0,
      explanation: `Перевод слова "${text}" на английский язык: ${translations[0].replace(/"/g, '\\"')}`,
    };
    const quizOptionsJson = JSON.stringify(quizOptions, null, 2);
    console.log(quizOptionsJson)
    ctx.replyWithQuiz(quizOptionsJson);
  });
